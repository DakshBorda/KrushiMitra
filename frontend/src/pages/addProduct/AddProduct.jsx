import React, { useState, useEffect } from "react";
import "./AddProduct.css";
import "react-date-range/dist/styles.css"; // main style file
import "react-date-range/dist/theme/default.css"; // theme css file

import { DateRangePicker } from "react-date-range";
import {
  createEquipment,
  getBrands,
  getEquipsList,
} from "../../api/equipments";
import item1 from "../../img/item1.png";
import { useSelector } from "react-redux";
import { useNavigate } from "react-router-dom";
import Cookies from "js-cookie";

const AddProduct = () => {
  const [visible, setVisible] = useState(false);
  const [startDate, setStartDate] = useState(new Date());
  const [equipmentTypes, setEquipmentTypes] = useState([]);
  const [brands, setBrands] = useState([]);
  const [endDate, setEndDate] = useState(new Date());
  const navigate = useNavigate();
  const [images, setImages] = useState({ image_1: null, image_2: null, image_3: null, image_4: null, image_5: null });
  const [imagePreviews, setImagePreviews] = useState({ image_1: null, image_2: null, image_3: null, image_4: null, image_5: null });
  const [data, setData] = useState({
    manufacturer: "",
    title: "",
    description: "",
    equipment_type: "",
    available_start_time: new Date().toISOString().slice(0, 10),
    available_end_time: new Date().toISOString().slice(0, 10),
    equipment_location: "",
    daily_rental: 0,
    hourly_rental: 0,
    manufacturing_year: 0,
    model: "",
    condition: "New",
    horsepower: 0,
    width: 0,
    height: 0,
    weight: 0,
  });

  const handleImageChange = (e, key) => {
    const file = e.target.files[0];
    if (file) {
      setImages((prev) => ({ ...prev, [key]: file }));
      setImagePreviews((prev) => ({ ...prev, [key]: URL.createObjectURL(file) }));
    }
  };

    useEffect(() => {
        if(!Cookies.get('access-token')) {
            navigate('/');
        }
    }, []);
  

  useEffect(() => {
    const loadOptions = async () => {
      try {
        const [typesRes, brandsRes] = await Promise.all([
          getEquipsList(),
          getBrands(),
        ]);
        if (typesRes?.data) {
          setEquipmentTypes(typesRes.data);
        }
        if (brandsRes?.data) {
          setBrands(brandsRes.data);
        }
      } catch (e) {
        console.log("Error loading equipment types/brands", e);
      }
    };
    loadOptions();
  }, []);

  const handleCreateEquipment = async (e) => {
    e.preventDefault();

    // Basic required-field validation before hitting the API
    if (!data.title || !data.description) {
      alert("Please fill in Title and Description.");
      return;
    }
    if (!data.manufacturer) {
      alert("Please select a Manufacturer.");
      return;
    }
    if (!data.equipment_type) {
      alert("Please select an Equipment Type.");
      return;
    }
    if (!data.equipment_location) {
      alert("Please enter Equipment Location.");
      return;
    }

    try {
      const res = await createEquipment(data, images);
      if (res?.data?.success) {
        alert(res.data.message);
        navigate("/dashboard");
      }
    } catch (err) {
      const errData = err?.response?.data;
      const errors = errData?.errors;
      const msg = errData?.message || errData?.msg || "Failed to create equipment.";
      if (errors && typeof errors === "object") {
        const parts = Object.entries(errors).map(([k, v]) => `${k}: ${Array.isArray(v) ? v.join(", ") : v}`);
        alert(`${msg}\n\n${parts.join("\n")}`);
      } else {
        alert(msg);
      }
    }
  };

  const handleSelect = (ranges) => {
    setStartDate(ranges.selection.startDate);
    setEndDate(ranges.selection.endDate);
  };
  const selectionRange = {
    startDate: startDate,
    endDate: endDate,
    key: "selection",
  };

  return (
    <div>
      <div className="my-10 max-w-6xl mx-auto">
        <div className="border-l-2 border-green-600 pl-6">
          <h1 className="text-2xl font-semibold text-gray-700">
            Describe Your Equipment
          </h1>
          <p className="text-md font-semibold mt-2 text-gray-500">
            Provide key details of your equipment to Sell Or Rent Out
          </p>
        </div>

        <form
          onSubmit={handleCreateEquipment}
          className="w-full mt-12 max-w-8xl"
        >
          <div className="flex flex-wrap -mx-3 mb-6">
            <div className="w-1/2 md:w-1/2 px-3 border p-2">
              <label
                for="exampleInputEmail1"
                className="form-label w-1/2 text-sm font-bold text-gray-500 inline-block pl-2 mb-2"
              >
                Title*
              </label>
              <input
                className="appearance-none block w-1/2 rounded py-3 px-2 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
                id="grid-last-name"
                type="text"
                placeholder="Enter equipment title"
                value={data.title}
                onChange={(e) => {
                  setData({ ...data, title: e.target.value });
                }}
              />
            </div>

            <div className="w-1/2 md:w-1/2 px-3 border flex flex-col p-2">
              <label
                for="exampleInputEmail1"
                className="form-label w-1/2 text-sm font-bold text-gray-500 inline-block pl-2 mb-2"
              >
                Manufacturer*
              </label>
              <select
                value={data.manufacturer || ""}
                onChange={(e) =>
                  setData({ ...data, manufacturer: e.target.value })
                }
                className="w-32"
              >
                <option value="" disabled>
                  Choose Brand
                </option>
                {brands.map((item, i) => {
                  return (
                    <option key={i} value={item.id}>
                      {item.name}
                    </option>
                  );
                })}
              </select>
            </div>
            <div className="w-1/2 md:w-1/2 px-3 border p-2">
              <label
                for="exampleInputEmail1"
                className="form-label w-1/2 text-sm font-bold text-gray-500 inline-block pl-2 mb-2"
              >
                Description*
              </label>
              <input
                className="appearance-none block w-1/2 rounded py-3 px-2 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
                id="grid-last-name"
                type="text"
                placeholder="Enter description"
                value={data.description}
                onChange={(e) => {
                  setData({ ...data, description: e.target.value });
                }}
              />
            </div>

            <div className="w-1/2 md:w-1/2 px-3 border p-2">
              <label
                for="exampleInputEmail1"
                className="form-label w-1/2 text-sm font-bold text-gray-500 inline-block pl-2 mb-2"
              >
                Daily Rental*
              </label>
              <input
                className="appearance-none block w-1/2 rounded py-3 px-2 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
                id="grid-last-name"
                type="text"
                placeholder="e.g. 500"
                value={data.daily_rental}
                onChange={(e) => {
                  setData({ ...data, daily_rental: e.target.value });
                }}
              />
            </div>
            <div className="w-1/2 md:w-1/2 px-3 border p-2">
              <label
                for="exampleInputEmail1"
                className="form-label w-1/2 text-sm font-bold text-gray-500 inline-block pl-2 mb-2"
              >
                Hourly Rental*
              </label>
              <input
                className="appearance-none block w-1/2 rounded py-3 px-2 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
                id="grid-last-name"
                type="text"
                placeholder="e.g. 100"
                value={data.hourly_rental}
                onChange={(e) => {
                  setData({ ...data, hourly_rental: e.target.value });
                }}
              />
            </div>

            <div className="w-1/2 md:w-1/2 px-3 border p-2 flex flex-col">
              <label
                for="exampleInputEmail1"
                className="form-label w-1/2 text-sm font-bold text-gray-500 inline-block pl-2 mb-2"
              >
                Equipment Type*
              </label>
              <select
                value={data.equipment_type || ""}
                onChange={(e) =>
                  setData({ ...data, equipment_type: e.target.value })
                }
                className="w-32"
              >
                <option value="" disabled>
                  Choose Equipment Type
                </option>
                {equipmentTypes.map((item, i) => {
                  return (
                    <option key={i} value={item.id}>
                      {item.name}
                    </option>
                  );
                })}
              </select>
            </div>

            <div className="w-1/2 md:w-1/2 px-3 border p-2">
              <label
                for="exampleInputEmail1"
                className="form-label w-1/2 text-sm font-bold text-gray-500 inline-block pl-2 mb-2"
              >
                Manufacturing Year*
              </label>
              <input
                className="appearance-none block w-1/2 rounded py-3 px-2 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
                id="grid-last-name"
                type="text"
                placeholder="e.g. 2020"
                value={data.manufacturing_year}
                onChange={(e) => {
                  setData({ ...data, manufacturing_year: e.target.value });
                }}
              />
            </div>
            <div className="w-1/2 md:w-1/2 px-3 border p-2">
              <label
                for="exampleInputEmail1"
                className="form-label w-1/2 text-sm font-bold text-gray-500 inline-block pl-2 mb-2"
              >
                Model*
              </label>
              <input
                className="appearance-none block w-1/2 rounded py-3 px-2 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
                id="grid-last-name"
                type="text"
                placeholder="e.g. Sonalika DI 745"
                value={data.model}
                onChange={(e) => {
                  setData({ ...data, model: e.target.value });
                }}
              />
            </div>

            <div className="w-1/2 md:w-1/2 px-3 border p-2 flex flex-col">
              <label
                for="exampleInputEmail1"
                className="form-label w-1/2 text-sm font-bold text-gray-500 inline-block pl-2 mb-2"
              >
                Condition*
              </label>
              <select
                value={data.condition}
                onChange={(e) =>
                  setData({ ...data, condition: e.target.value })
                }
                className="w-32"
              >
                <option value="New">New</option>
                <option value="Used">Used</option>
              </select>
            </div>

            <div className="w-1/2 md:w-1/2 px-3 border p-2">
              <label
                for="exampleInputEmail1"
                className="form-label w-1/2 text-sm font-bold text-gray-500 inline-block pl-2 mb-2"
              >
                Equipment Location*
              </label>
              <input
                className="appearance-none block w-1/2 rounded py-3 px-2 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
                id="grid-last-name"
                type="text"
                placeholder="e.g. Ahmedabad, Gujarat"
                value={data.equipment_location}
                onChange={(e) => {
                  setData({ ...data, equipment_location: e.target.value });
                }}
              />
            </div>

            <div className="w-1/2 md:w-1/2 px-3 border p-2">
              <label
                for="exampleInputEmail1"
                className="form-label w-1/2 text-sm font-bold text-gray-500 inline-block pl-2 mb-2"
              >
                HorsePower*
              </label>
              <input
                className="appearance-none block w-1/2 rounded py-3 px-2 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
                id="grid-last-name"
                type="text"
                placeholder="e.g. 50"
                value={data.horsepower}
                onChange={(e) => {
                  setData({ ...data, horsepower: e.target.value });
                }}
              />
            </div>

            <div className="w-1/2 md:w-1/2 px-3 border p-2">
              <label
                for="exampleInputEmail1"
                className="form-label w-1/2 text-sm font-bold text-gray-500 inline-block pl-2 mb-2"
              >
                Height*
              </label>
              <input
                className="appearance-none block w-1/2 rounded py-3 px-2 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
                id="grid-last-name"
                type="text"
                placeholder="e.g. 150 cm"
                value={data.height}
                onChange={(e) => {
                  setData({ ...data, height: e.target.value });
                }}
              />
            </div>
            <div className="w-1/2 md:w-1/2 px-3 border p-2">
              <label
                for="exampleInputEmail1"
                className="form-label w-1/2 text-sm font-bold text-gray-500 inline-block pl-2 mb-2"
              >
                Width*
              </label>
              <input
                className="appearance-none block w-1/2 rounded py-3 px-2 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
                id="grid-last-name"
                type="text"
                placeholder="e.g. 200 cm"
                value={data.width}
                onChange={(e) => {
                  setData({ ...data, width: e.target.value });
                }}
              />
            </div>

            <div className="w-1/2 md:w-1/2 px-3 border p-2">
              <label
                htmlFor="weight"
                className="form-label w-1/2 text-sm font-bold text-gray-500 inline-block pl-2 mb-2"
              >
                Weight (lbs)*
              </label>
              <input
                className="appearance-none block w-1/2 rounded py-3 px-2 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
                id="weight"
                type="text"
                placeholder="e.g. 2500"
                value={data.weight}
                onChange={(e) => {
                  setData({ ...data, weight: e.target.value });
                }}
              />
            </div>

            <div className="w-full px-3 border p-4 mt-4">
              <label className="form-label text-sm font-bold text-gray-500 inline-block pl-2 mb-3">
                Equipment Images (upload up to 5)
              </label>
              <div className="flex flex-wrap gap-4">
                {["image_1", "image_2", "image_3", "image_4", "image_5"].map((key, i) => (
                  <div key={key} className="flex flex-col items-center">
                    <img
                      src={imagePreviews[key] || item1}
                      alt={`Preview ${i + 1}`}
                      className="w-24 h-24 object-cover rounded border mb-2"
                      style={{ opacity: imagePreviews[key] ? 1 : 0.3 }}
                    />
                    <input
                      type="file"
                      accept="image/*"
                      onChange={(e) => handleImageChange(e, key)}
                      className="text-xs w-28"
                    />
                    <span className="text-xs text-gray-400 mt-1">Image {i + 1}</span>
                  </div>
                ))}
              </div>
            </div>

            <div className="w-full md:w-1/2 px-3 mb-6 border p-2 md:mb-0">
              <label
                htmlFor="dateRange"
                className="form-label text-lg bg-gray-200 hover:bg-gray-300 font-bold text-gray-500 inline-block pl-2 mb-2"
                onClick={() => setVisible(true)}
              >
                Select Date Range
              </label>
              {visible ? (
                <DateRangePicker
                  style={{ height: "300px", width: "280px" }}
                  ranges={[selectionRange]}
                  minDate={new Date()}
                  rangeColors={["#68AC5D"]}
                  onChange={handleSelect}
                />
              ) : (
                <div></div>
              )}
            </div>
          </div>
          <button
            type="submit"
            className="mt-32 bg-darkgreen ml-8 hover:bg-green-700 text-white font-bold py-2 px-8 rounded-full mx-auto"
          >
            Add Equipment
          </button>
        </form>
      </div>
    </div>
  );
};

export default AddProduct;
